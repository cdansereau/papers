...
\paragraph{Validity of parametric model} In order to test the possible violations of the assumption of homoscedasticity exist over centers we performed a White test over each connection pair to make sure that the standard errors are not biased by heteroscedastic connections. Because the standard error is central to conducting significance tests and calculating confidence intervals, biased standard errors lead to incorrect conclusions about the significance of the regression coefficients. We found that some connections were rejecting the heteroscedastic hypothesis with an effect size between 0.12 and 0.22 for an average of 0.19 (for more details see Supplementary Material \ref{fig_hetero}), this effect size would be deemed small-to-moderate \citep{Cohen1992}. Although as shown in Figure \ref{fig_h0} the false positive were controled perfectly under the null hypothesis, therefore the p-values shown in this study are valid and well controlled, even though some connections were found to be heteroscedastic. Overall, the violation of the homoscedasticity assumption must be quite severe in order to present a major problem given the robust nature of ordinary least-squares regression.   

\paragraph{Other parametric models} Other models could have been used that may be more robust to heteroscedasticity like weighted least squares regression or linear mixed-effects modeling \citep{Chen2013} but for the sake of simplicity we evaluated the site effect with two simple models: one univariate (ordinary least squared regression) and one multivariate (SVM) since the heteroscedasticity is not large enought to cause problem in false discovery those simple models are safe to use.
