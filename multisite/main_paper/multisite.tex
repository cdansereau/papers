...
\begin{itemize}
 ...
 \item $(\mathcal{E}_3)$ Test the impact of multisite modelization and affected connection volume ($\pi_1$) on the prediction accuracy, For the prediction scenario we used a range of $\pi_1\in\{0.1,1,5\%\}$, $120$ subjects for training and $28$ for testing.
  \item $(\mathcal{E}_4)$ Test the impact of multisite modelization and affected connection volume ($\pi_1$) on the prediction accuracy, For the prediction scenario we used a range of $\pi_1\in\{0.1,1,5\%\}$, $80$ subjects for training and $68$ for testing.
\end{itemize}
...
Effect size of the inter-site bias from a subset of 8 sites from the 1000 FCP. Panel A shows the distribution of absolute difference in functional connectivity across all brain connections. Panel B shows the distribution of of inter-site bias for each connection, expressed as Cohen's $d$.
}
...
Effect size of the inter-site bias from a subset of 8 sites from the 1000 functional connectome dataset for the selected 11 functional connections. A) distribution of absolute effect for each connections B) distribution of inter-site bias for each connection, expressed as Cohen's $d$.
}
