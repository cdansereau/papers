...
\begin{itemize}
 ...
 \item $(\mathcal{E}_3)$ Test the impact of multisite modelization and affected connection volume ($\pi_1$) on the prediction accuracy, For the prediction scenario we used a range of $\pi_1\in\{0.1,1,5\%\}$, $n_{training}=120,n_{testing}=28$,
  \item $(\mathcal{E}_4)$ Test the impact of multisite modelization and affected connection volume ($\pi_1$) on the prediction accuracy, For the prediction scenario we used a range of $\pi_1\in\{0.1,1,5\%\}$,  $n_{training}=80,n_{testing}=68$,
\end{itemize}
...
Effect size of the inter-site bias from a subset of 8 sites from the 1000 FCP. Panel A shows the distribution of absolute difference in functional connectivity across all brain connections. Panel B shows the distribution of effect size for each connections expressed in Cohen's $d$ (absolute effect divided by the standard deviation of the noise).
}
