...
At 0.1\% (around 5 connections) the accuracy level was at the level of chance, starting at $\pi_1=1\%$ the 
- comparing the monosite and the multisite we have a sentential drop of accuracy from monosite to multisite but this will be to the benefit of generalization, which is an acceptable drawback. an other point is the fact that to achieve a 95\% accuracy both monosite and multisite need the same size of effect. The monosote is more linear in it's relationship to effect-size and the multisite in behaving more like a sigmoidal curve reaching the same level of accuracy as the monosite at hight accuracy level.
- As expected the model require a minimum of connection that depict the effect in order to be detectable 
- it also need a larger effect size to to achieve a classification accuracy in the range of 90\%
-Correcting for site-effect by regressing the out the dummy variable do not make a difference for multivariate analysis as compared to univariate analysis who do not control correctly for false positive.
Figure \ref{fig_prediction_sampeffect}
