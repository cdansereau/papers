...
\begin{abstract}
\textit{Purpose:} To estimate multisite bias in functional MRI (fMRI) and its impact on our capacity to detect effects in resting state (rs) functional connectivity measures. \emph{This is too technical: what does bias mean? What exactly means multisite? you can express the same ideas without jargon and just a few extra words. Then, once the jargon has been properly defined, run with it}\\
\textit{Materials and Methods:} We investigated real fMRI datasets collected across eight scanning sites with 3T scanners and heterogeneous scanning protocols, drawn from the 1000 functional connectome project. We first used this sample to characterize the amplitude of site bias in connectivity measures, as well as how this bias was distributed across brain networks. We then implemented a series of Monte-Carlo simulations, based on real data, to evaluate the impact of multisite bias on statistical power in statistical tests comparing two groups with the general linear model. As a reference, we also implemented the same simulations with fMRI data collected at a single site, with sample size equivalent to that of all the multisite data combined.\\
\textit{Results:} Inter-site variability of fMRI data contributed 3 time less than the total variance than the inter-subject variance \emph{Not sure what this sentence mean. Try to formulate the result rigorously}. Simulations revealed that a systematic bias exist due to multisite aquisition although this difference thend to diminish as we increase the sample site to a point where the difference between a monosite and a multisite dataset are negligible \emph{Bias is different from reduced sensitivity. The bias is not impacted by the sample size. The decrease in sensitivity is}.\\
\textit{Conclusion:} These results support the feasability of multisite dataset and in particular of pooled datasets from heterogeneous nature \emph{I think it's great to have a ``big picture'' summary at the end of the abstract. But this is too vague. ``the feasability of multisite dataset'' for example means that you are able to collect data at multiple sites. This is not what you actually mean here. Please rephrase, keep it general but make it accurate}.
\end{abstract}
