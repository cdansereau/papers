...
Panel A: map of the default mode network (DMN) obtained using a seed in the posterior cingulate cortex (PCC) from the average connectomes of all the sites. Panels B,C:  The first column show the average functional connectivity maps of the default-mode network at 8 sites. The second column show the significant differences between the average functional connectivity maps of one site versus all the others. Panel D shows the number of sites with a significant inter-site difference for each brain region.
}
