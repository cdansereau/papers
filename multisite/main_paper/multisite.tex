...
\subsection{Monte-Carlo simulations of statistical power}
We evaluated the statistical power of a test of group difference in rs-fMRI connectivity, with either a single or multiple sites of data collection. Figure \ref{fig_real_sim_samplesize} shows the effect of the sample size on the detection power when the allocation ratio $W$ is 50\%, i.e. the same number of patients as controls for every site. Note that the total sample size $N$ was identical for the single site and multisite simulations, and all 7 sites had a fixed number of subjects ($N/7$). As expected, the detection power increased with the overall sample size, both for single-site and multisite tests: for a large effect size of 1, significant differences ($\alpha < =0.001$) were detected in only 20\% of the simulations with 40 subjects, in 80\% of the simulations with 80 subjects and in almost 95\% of the simulations with 120 subjects. 
