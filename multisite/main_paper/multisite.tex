...
%and suggest to use a total sample size larger than $N=100$.
\end{abstract}
...
%We ran simulations on 11 connections likely impacted by Alzheimer's disease and showing a fair-to-good test-retest reliability
...
across all sites. 
%Some significant site effects were still detected in the connectivity both within each network, as well as in the off-diagonal rectangles highlighting between-network connectivity. By counting the frequency of sites showing a significant effect across sites, it was apparent that significant site effects were quite variable in their localization and spread across the full connectome.
...
\paragraph{Parametric model validity} 
We tested the model for heteroscedasticity and unsurprisingly we found that some connections are found to be significant when we perform a White test on them. 
see in Supplementary Material \ref{fig_hetero}.
...
\end{center}
\caption[]{
Panel A show test of heteroscedasticity. Panel B show the size of the average absolute standard deviation difference between sites, and Panle C show the normalized diference also refered has the effect size. 
}
\label{fig_hetero}
\end{figure}
