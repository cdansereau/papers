...
We implemented Monte-Carlo simulations to assess the sensitivity of detection of group differences in multisite rs-fMRI connectivity. The simulations were based on the 1000 FCP sample, with 8 sites for a total of 345 subjects. The multisite simulations were based on 7 sites, totalling 148 subjects, and the monosite simulations were based on the Cambridge site, with 195 subjects. The following data generation model was used to simulate tests of group difference. A subsample with a fixed number of subjects was first selected randomly at each site. For each site, a ratio $W$ of the selected subjects were randomly assigned to a ``patient'' group. We focussed the simulations on 11 connections likely impacted by Alzheimer's disease and and with a fair to good test-retest reliability, see \cite{Orban2015} for details. For the subjects in the ``patient'' group, and for each connection, a fixed value was added to the connectivity measure to achieve a given effect size in the group comparison, as defined below. 
...
In addition to mass univariate GLM tests, we also investigated a linear SVM \citep{Cortes1995} using the same Monte Carlo simulations. For this technique, all possible connections between the 100 brain parcels were used simultaneously to predict the presence of the simulated pathology in a given subject. In this simulation setting, a ``pathology'' effect was only simulated in a certain (random) percentage of the connections, denoted as $\pi_1$. The SVM model was trained on a randomly selected subset of subjects denoted by $n_{training}$ and tested on the remaining $n_{testing}$ subjects of the total 148 subjects. Using 10-fold cross-validation we optimized the ``C'' hyper-parameters of the SVM independently for each of the $B$ simulations. Average accuracy and standard deviation of the accuracy score over the $B$ simulation are computed for each effect size and $\pi_1$. 

% The simulation scenarios
\paragraph{Simulation scenarios}
For each experiment using real data, all effect size in the range 0 to 2 Cohen’s $d$ with a step of 0.01 were considered. For each experiment we performed $B=10^3$ simulations using sub-sampling and the reported allocation ratio of participants $W$ and its inverse $1-W$ was attributed randomly to the ``patient'' population for each site. The sub-sampling scheme used was obtained by calculating the sampling ratio desired sample-size / total subject available from all sites. We then extracted a random sample of each site based on this ratio. We implemented the following experiments:
...
\begin{itemize}
 ...
 \item The same prediction analysis was perform although only training on $n_{training}=80$ subjects and testing on the rest of the 148 subjects ($n_{testing}=68$ subjects).
\end{itemize}
