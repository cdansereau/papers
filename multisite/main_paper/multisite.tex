...
Quantify
\paragraph{Inter-site bias in rs-fMRI connectivity}  Typical networks were reliably found across sites like the default mode network (DMN) which is an expected findings inline with the literature \citep{Zhang2012,Damoiseaux2006,Bellec2010c,Buckner2008,Fransson2005,Fransson2006}. Other networks like the so called consistent resting state networks (CRSNs) as shown in \cite{Damoiseaux2006, Bellec2010c, Dansereau2014} are also consistently found across sites. These CRSNs are consistent regardles of the method use to extract them since the same finding have been reproduce using independent component analysis (ICA) \citep{Damoiseaux2006} and clustering \citep{Bellec2010c}.\emph{Was that expected? Is that in line with some findings of the literature? Where there other networks than the DMN? Is this finding also robust to the methods used to extract networks?}. We found that significant differences exist that could undermine the interpretation of the results \emph{Which results? this is very vague}. This work confirmed the existence of an inter-site connectivity bias \emph{Was that expected? }. The identified location of those significant effect were quite variable and most of them identified in less than half the sites \emph{Implications? Are there some ``better'' networks when it comes to site bias?}. The inter-subject (intra-site) contribution in term of variance was found to be 3 fold greater than the inter-site contribution. This effect size would be deemed small-to-moderate, which suggests that the impact of additive inter-site bias on statistical tests will be limited. A reassuring finding in favour of multisite pooling.\\
