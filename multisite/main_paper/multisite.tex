...
\begin{abstract}
Studies based on connectivity measures derived from functional magnetic resonance imaging are increasingly pooling data acquired at multiple sites. While this may allow investigators to speed up recruitment or increase sample size, multisite studies also potentially introduce systematic biases in connectivity measures across sites. In this work, we quantified the inter-site bias in connectivity and its impact on the detection of group differences. Our study was based on real multisite fMRI datasets collected in $N=345$ young, healthy subjects across eight scanning sites with 3T scanners and heterogeneous scanning protocols, drawn from the 1000 functional connectome project. We first empirically showed that typical functional networks were reliably found at the group level in all sites, and that the amplitude of the inter-site bias was small to moderate, with a Cohen's effect size below 0.5 on average across brain connections. We then implemented a series of Monte-Carlo simulations, based on real data, to evaluate the impact of the multisite bias on detection power in statistical tests comparing two groups (with and without the effect) using a general linear model, as well as on the prediction of group labels with a support-vector machine. As a reference, we also implemented the same simulations with fMRI data collected at a single site using an identical sample size. Simulations revealed that using heterogeneous sites slightly decreased our ability to detect changes compared to a monosite study with the GLM, and had a more serious impact in prediction accuracy. However, these differences tended to decrease as the total sample size increased, to a point where differences between monosite and multisite simulations were small with $N=120$ subjects. Taken together, our results support the feasibility of multisite studies in rs-fMRI and suggest to use a total sample size larger than $N=100$.
\end{abstract}

%-- 
\begin{keyword}
multisite study \sep statistical power \sep prediction accuracy \sep Monte-Carlo simulation \sep sample size \sep resting-state \sep fMRI connectivity
...
brain parcels were used simultaneously to predict the presence of the simulated
...
Panel A: map of the DMN obtained using a seed in the posterior cingulate cortex (PCC), averaging all subjects and sites together. Panels B,C:  The first column shows the average functional connectivity maps of the DMN at 8 sites. The second column shows the significant differences between the average functional connectivity maps of one site versus all the others. Panel D shows the number of sites with a significant inter-site difference for each brain region.
}
\label{fig_DMN_variability}
\end{figure}

\paragraph{Site bias in the default-mode network} We first focused on the
connections associated with a seed region located in the precuneus (PCC), a key
...
\ref{fig_DMN_variability}. Figure \ref{fig_DMN_variability}A shows the PCC
connectivity map, averaged across all subjects and all sites. The key regions of
the DMN are easily identifiable, and include the PCC, precuneus, inferior
...
\ref{fig_DMN_variability}B,C. Qualitatively, the DMN maps were consistent across
...
\ref{fig_DMN_variability}B,C. Figure \ref{fig_DMN_variability}D shows how
reproducible were the significant biases in connectivity across the brain and
sites. The identified connections were quite variable, most of them being
...
corresponds to the intra-network connectivity for a partition into 7 networks
presented in Figure \ref{fig_connectome_variability}D. These 7 networks were
...
networks (dATT and vATT, respectively), mesolimbic and cerebellar networks were
identified. Figure \ref{fig_connectome_variability}C shows how this large-scale
...
($q\leq 0.05$) is presented in Figure \ref{fig_connectome_variability}C.
...
across all sites. Some significant site effects were still detected in the
connectivity both within each network, as well as in the off-diagonal rectangles
highlighting between-network connectivity. By counting the frequency of sites
showing a significant effect across sites, it was apparent that significant site
effects were quite variable in their localization and spread across the full
connectome.  
...
Effect size of the inter-site bias from a subset of 8 sites from the 1000 FCP. Panel A shows the distribution of absolute differences in functional connectivity across all brain connections. Panel C shows the same distribution across the selected 11 functional connections only. Panel B shows the distribution of inter-site biases for each connection, expressed as Cohen's $d$. The same distribution is shown for across the selected 11 functional connections in Panel D.
}
\label{fig_site_variability}
\end{figure}
%Distribution of intra-site (between-subject) standard deviation vs. inter-site (between-site) standard deviation, from a subset of 8 sites from the 1000 FCP dataset. A) Intra-site standard deviation for each site. B) Inter-site effect size expressed in Cohen's $d$ for each site.
...
connections from the Cambridge parcellation. Figure \ref{fig_site_variability}C
...
\ref{fig_std_connectomes}. Figure \ref{fig_site_variability}B presents a violin
...
connections that were used in this study, as shown in Figure
\ref{fig_site_variability}D. The effect size of the inter-site connectivity bias
was of the order of 0.3 Cohen's $d$. This effect size would be deemed small-tomoderate,
...
\paragraph{Inter-site bias in rs-fMRI connectivity} Typical resting-state networks were reliably found across sites, such as the DMN, the vATT, dATT, visual and sensorimotor networks. This was strongly expected given the relative consistency of their distribution across individuals, studies, preprocessing approaches or even methods used to extract networks \citep[e.g.][]{Damoiseaux2006,Van_den_Heuvel2008-hq,Bellec2010c,Yeo2011,Power2011}. We however found that significant differences
in average connectivity existed between sites (i.e. site biaises), as previously
reported by \citep{Yan2013a}. This connectivity bias may undermine the
...
twice as large as the inter-site absolute bias, on average across brain connections. This effect size measured in Cohen's d would be deemed small-to-moderate, which suggests that the impact of additive inter-site bias on statistical tests will be limited. This is a reassuring finding supporting the feasibility of statistical tests pooling fMRI data across multiple sites. Previous studies \cite{Sutton2008,Brown2011} had reported inter-site variance up to 10 times smaller than inter-subject variability, but these studies had much more homogeneous scanning environments than ours and also used different fMRI outcome measures. In our case, we still investigated only 3T scanners, mostly Siemens TIM TRIO, and inter-site bias may be larger when considering other manufacturers or field strengths.\\

%\paragraph{Spatial distribution of connectivity bias} We found that the brain connections exhibiting a significant inter-site bias were quite variable, with most of them being identified in less than one fourth of the sites. This suggests that there are no large network-specific inter-site bias or that the statistical power for detecting bias with the employed site sample size was limited. Looking at the spatial location of the site bias with respect to a brain parcellation with 7 networks, we noticed that the DMN and the dATT networks were less affected by site bias then the cerebellar, mesolimbic and sensorimotor networks \emph{did we really observe that? CD: Yes looking at the matrix of figure2D I could observe a bit more difference from one network to the other I can compute the average of the value in each network that would give use a cleaner picture... }. This last finding may be associated with the fact that those network are located on the periphery of the brain and may be more sensitive to motion artifacts. 
...
Comparing the monosite and the multisite accuracy curves we have a substantial
...
the monosite sample. A somewhat surprising observation was that linear
...
important step to improve our sensitivity to small effect sizes.
