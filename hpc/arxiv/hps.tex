...
\caption{Demeaned gray matter volume measures of the right hemisphere. Panel A shows individual maps and the correlation of every subject with all other subjects in Panel B. Panel C shows the subtypes templates representing subgroups in the dataset.}
\label{fig_subj_var}
\end{figure*}
...
\caption{Count of the number of subject in each group: low (L), moderate (M), high (H) risk for each clinical categories (CN: cognitively normal, SCI: subjectivecognitive impaiment, MCI: mild cognitive impairment, AD: Alzheimer's dementia). The bars size represent the proportion in pourcentage of subject in that clinical category and the number on top of each bar represent the absolute count.}
\label{fig_hitproba}
\end{figure*}
...
\caption{Distribution of CDRSB scores for each group: low (L), moderate (M), high (H) risk devided by there clinical categories (CN: cognitively normal, SCI: subjectivecognitive impaiment, MCI: mild cognitive impairment, AD: Alzheimer's dementia)}
\label{fig_hitproba}
\end{figure*}
...
\caption{Longitudinal followup of the three groups: low (L), moderate (M), high (H) risk. First column show CDR-Sb score, second show de delta of CDR-SB with respect to baseline. Error bars represent the standard error of the mean. Each column represent the groups obtain from each configuration (fMRI, sMRI and fMRI+sMRI).}
\label{fig_hitproba}
\end{figure*}
